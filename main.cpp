...
void MassToFile(int **a, int n, std::string file1, int k)
{
	char m;
	std::ofstream file;
	file.open(file1 + ".txt");
...
int main()
{
	...
	figure figure[32], choice[6], menu[5];
	Clock clock;
	double dis, timer;
	...
	int trigger_map = 0, trigger_figure = -1, trigger_figure_to = -1, trigger_view=180, trigger_talk = -1,
		trigger_choice = -1, trigger_menu = -1;
	...
	//RenderWindow Menu(VideoMode(*xwin, *ywin), "Menu");
	view.reset(sf::FloatRect(0, 0, *xwin, *ywin));
	menu[4].initial('1', 1100, 600);
	menu[4].setemojanot("Exit");
	menu[4].reload();
	menu[0].text.setCharacterSize(16);
	menu[0].text.setFont(font);
	menu[0].initial('1', 200, 200);
	menu[0].settext("Continue");
	menu[0].reload();
	menu[1].text.setCharacterSize(16);
	menu[1].text.setFont(font);
	menu[1].initial('1', 200, 300);
	menu[1].settext("New Game");
	menu[1].reload();
	while (window.isOpen())
	{
		a = new int*[10];
		...
		while (window.pollEvent(event))
		{
			if ((event.key.code == Mouse::Left) && (mousec == false))
			{
				if(trigger_menu==-1)
					for (int i = 0; i < 5; i++)
					{
						if ((menu[i].talk_window.getGlobalBounds().contains(wPos.x, wPos.y))|| (menu[i].sprite.getGlobalBounds().contains(wPos.x, wPos.y)))
						{
							mousec = true;
							trigger_menu = i;
						}
			...
			if (trigger_menu == 4) window.close();
			...
			while (window.isOpen())
			{
				...
				while (window.pollEvent(event))
				{
					if ((event.key.code == Mouse::Left) && (mousec == false) && (figure[trigger_figure].isMove != true) && (figure[trigger_figure].isAttack != true))
					{
						if (trigger_menu == -1)
						for (int i = 0; i <= 4; i++)
							if (menu[i].sprite.getGlobalBounds().contains(wPos.x, wPos.y))
							{
								mousec = true;
								trigger_menu = i;
							}
				...
				if (trigger_view < 180)
				{
					...
					ChangeView(menu, 5, k, x_desk + 200, y_desk + 200, trigger_view);
				}
				...
				window.draw(menu[4].sprite);
				for (int i = 0; i <= 32; i++)
					if (figure[i].pass == 1)
					{
						figure[i].reload();
						window.draw(figure[i].sprite);
						if ((w == 2) && (figure[i].value / 7 == 0)) figure[i].pass = 0;
						else if ((b == 2) && (figure[i].value / 7 == 1)) figure[i].pass = 0;
		...
		for (int i = 0; i < 4; i++)
		{
			...
			window.draw(menu[i].talk_window);
			window.draw(menu[i].text);
		}
		window.draw(menu[4].sprite);
		window.display();
	}
